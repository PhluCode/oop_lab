...
public class LibraryItem {
    ...
    public void checkOut(Member member) {
        ...
        if (member.canBorrow()){
            ...
            int loanDays = member.getStrategy().getLoanPeriodDays();
            this.returnDate = LocalDate.now().plusDays(loanDays);

            System.out.println(" Book ["+ this.title + "] has been checked out successfully ");
            System.out.println(" Book ["+ this.title + "] has been borrowed by " + member.getMemberName() + ".");
    ...
    public boolean isAvailable() {
        if (this.isAvailable.equalsIgnoreCase("available")) {
            return true;
        } else  {
            return false;
        }
